...
class SuperannuationInference:
    def __init__(self, models_dir: str = "models", csv_path: str = "case1.csv"):
    ...
    def load_data(self):
        ...
        print(f"Data loaded: {len(self.df)} users")
    ...
    def get_user_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        # Add a generated name based on User_ID for better UX
        ...
        for feature in clustering_features:
            if feature in user:
                if feature == 'Risk_Tolerance':
                    ...
                    risk_levels = ['Low', 'Medium', 'High']
                    feature_values.append(risk_levels.index(user[feature]) if user[feature] in risk_levels else 1)
                else:
                    feature_values.append(user[feature])
            else:
    ...
    def get_peer_statistics(self, user_id: str, cluster: int) -> Dict[str, Any]:
        ...
        cluster_users = self.df[self.df['Cluster'] == cluster] if 'Cluster' in self.df.columns else self.df
        ...
        peer_stats = {
            ...
            'avg_contribution': float(cluster_users['Contribution_Amount'].mean()),
            'common_investment_types': cluster_users['Investment_Type'].value_counts().head(3).to_dict(),
            'common_fund_names': cluster_users['Fund_Name'].value_counts().head(3).to_dict(),
            'risk_distribution': cluster_users['Risk_Tolerance'].value_counts().to_dict(),
            'contribution_percentile': float((cluster_users['Contribution_Amount'] <= user['Contribution_Amount']).mean() * 100)
        }
