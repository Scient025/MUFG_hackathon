...
async def startup_event():
    ...
    try:
        ...
        print("API startup complete - ML models and chat router loaded!")
...
async def get_peer_statistics(user_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting peer stats: {str(e)}")

@app.post("/simulate")
async def simulate_scenario(request: SimulationRequest):
    """Run new projection with adjusted contributions"""
    try:
        print(f"Simulating scenario for user {request.user_id} with extra monthly: {request.extra_monthly}")
        projection = inference_engine.predict_pension_projection(
            request.user_id, 
            request.extra_monthly
        )
...
async def signup_user(request: UserSignupRequest):
    ...
    try:
        ...
        new_user_data = {
            ...
            'Savings_Rate': request.contribution_amount / request.annual_income if request.annual_income > 0 else 0,
            'Debt_Level': 'Low'  # Default assumption
        }
        
        # Add to dataframe
        new_row = pd.DataFrame([new_user_data])
        inference_engine.df = pd.concat([inference_engine.df, new_row], ignore_index=True)
...
async def get_all_users():
    ...
    try:
        ...
        available_columns = ['User_ID', 'Age', 'Risk_Tolerance', 'Annual_Income', 'Current_Savings']
        
        # Check which columns actually exist
        existing_columns = [col for col in available_columns if col in inference_engine.df.columns]
